...
public class SmartUtils {
    ...
    public static String readJsonAsString(String scenarioFile) {
        try {
            ...
            LOGGER.error("[CCS Master] : Exception occurred while parsing the 'Test Scenario' file:{}. " +
    ...
    public static List<String> getAllEndPointFiles(String packageName) {
        ...
        if (null == allSimulationFiles || allSimulationFiles.length == 0) {
            LOGGER.error("[CCS Master] : Test folder is empty or not correctly setup.");
    ...
    public static List<String> retrieveScenariosByAbsPath(String packageName) {
        ...
        try {
            ...
            LOGGER.error("[CCS Master] : Exception during reading absolute suite folder - " + exx);
        ...
        getScenarioSpecListByPackage(testPackageName).stream()
                .forEach(scenarioSpec -> {
                    if (!oops.add(scenarioSpec.getScenarioName())) {
                        throw new RuntimeException("[CCS Master] : Can not run with multiple Scenarios with same name. Found duplicate: " + scenarioSpec.getScenarioName());
                    }
    ...
    public static String prettyPrintJson(String jsonString) {
        ...
        try {
            ...
            LOGGER.error("[CCS Master] : Non-JSON content was encountered. So pretty print did not format it and returned the raw text");
            return jsonString;
        }
